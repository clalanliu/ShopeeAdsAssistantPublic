...
with col1:
    ...
    personalization["saa_sub_id"] = st.text_input(ui_text_df[lang][1], value=personalization["saa_sub_id"])
    personalization["saa_sub_key"] = st.text_input(ui_text_df[lang][2], value=personalization["saa_sub_key"])
    personalization["chrome_profile_data"] = st.text_input(ui_text_df[lang][3], value=personalization["chrome_profile_data"])
    personalization["uuid"] = personalization["uuid"] if not pd.isnull(personalization["uuid"]) else subprocess.check_output('wmic csproduct get uuid'.split(), shell=True).decode('utf-8').split()[1]
    ...
    personalization["id"] = st.text_input(ui_text_df[lang][8], value=personalization["id"])
    personalization["pwd"] = st.text_input(ui_text_df[lang][9], type="password", value=personalization["pwd"])
...
with col2:
    ...
    if check_subscription(personalization):
        ...
        edited_df = st.experimental_data_editor(item_list, num_rows="dynamic")
        ...
        selected_options = st.multiselect(ui_text_df[lang][18], edited_df['item'])
...
with col3:
    if check_subscription(personalization):
        ...
        selected = st.selectbox(ui_text_df[lang][22], list_exist_items(summary_directory))
        
        if selected:
            generate_table_button = st.button(ui_text_df[lang][23])
            if selected and generate_table_button:
                df = pd.DataFrame(list_exist_items_from_json(os.path.join(summary_directory, selected, 'new_keyword_bid.json')))
...
with col4:
    ...
    if check_subscription(personalization):
        ...
        selected_options = st.multiselect(ui_text_df[lang][25], item_list['item'], key="set_bid_multiselect")

        if len(selected_options) > 0:
            set_bid_button = st.button(ui_text_df[lang][26], key="set_bid_button")
